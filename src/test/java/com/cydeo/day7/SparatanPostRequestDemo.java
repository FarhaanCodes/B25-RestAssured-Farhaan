...
public class SparatanPostRequestDemo extends SpartanTestBase {
 ...
 */
    ...
    public void test1(){
       ...
       Response response = given().accept(ContentType.JSON).log().all() // what we are asking from api which is JSON response
                ...
                .body(requestBody)
                .when()
                .post("/api/spartans"); //what we are sending to api, which is JSON request body
    ...
    public void test4(){
        ...
        Spartan spartanGet = given().accept(ContentType.JSON)
                ...
                .get("/api/spartans/{id}")
                .then()
                .statusCode(200).log().all().extract().response().as(Spartan.class);
